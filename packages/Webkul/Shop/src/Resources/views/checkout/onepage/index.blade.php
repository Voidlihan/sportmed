        ...
        >
            ...
            >
                <div class="bg-white p-6 rounded shadow-xl w-full max-w-md text-center">
                    <h2 class="text-xl font-semibold mb-4">Отсканируйте QR-код Kaspi или Halyk для оплаты</h2>

                    <!-- Тут можно вставить QR-код как картинку -->
                    <div v-if="qrImages.length">
                        <div
                            v-for="(img, index) in qrImages"
                            :key="index"
                            class="mb-4"
                        >
                            <img
                                :src="img.url"
                                class="mx-auto mb-2 max-h-64"
                                :alt="img.title"
                    ...
                    <div class="flex justify-center mt-4" style="background: black; margin-left: 630px; width: 215px;">
                        <button
            ...
            app.component('v-checkout', {
                ...
                data() {
                    return {
                        ...
                        showQRModal: false, // 👈 Добавлено для модального окна
                ...
                methods: {
                    ...
                    // 🟡 Эта кнопка теперь просто открывает модалку
                    placeOrder() {
                        this.showQRModal = true;
                    },
                    // ✅ После нажатия в модалке — заказ оформляется
                    confirmQRPayment() {
                    ...
                    loadQRImages() {
                        this.$axios.get('/qr-images-db')
                            .then(response => {
                                console.log('QR Images:', response.data); // <- вот это
                                this.qrImages = response.data;
                            })
                            .catch(error => {
                                console.error('Ошибка при загрузке QR-изображений:', error);
                            });
                    },
                },
            });
